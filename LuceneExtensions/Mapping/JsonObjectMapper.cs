...
namespace Lucene.Net.Mapping
{
    ...
    public sealed class JsonObjectMapper : IObjectMapper
    {
        ...
        private static void Add(Document doc, string prefix, JToken token)
        {
            if (token is JObject)
            {
                AddProperties(doc, prefix, (JObject) token);
            }
            else if (token is JArray)
            {
                AddArray(doc, prefix, (JArray) token);
            }
            else if (token is JValue)
            {
                var value = token as JValue;

                // if (value != null && value!.Type == JTokenType.String && (string) value.Value! == "Id")
                // {
                //     doc.Add(new StringField(prefix, value.Value!.ToString(),Field.Store.YES));
                //     return;
                // }
